...
- The code checkout was clean before this review. The separate task workspace named `neuralDOOM` contains an empty Git repository; implementation belongs in this RBDOOM checkout.
...
The September 1-3 entries in `TEST_RESULTS.md` document baseline DX12, camera/rigid/skinned/viewmodel motion, classification masks, shared history resets, the neutral backend, native Streamline DLAA, and user-observed embedded NR gameplay. These are historical results, not fresh runtime validation from this review.
...
| Order / ID | Deliverable and concrete scope | AFK acceptance evidence |
|---|---|---|
...
- Update task board and test notes per meaningful change; run focused diff review and `git diff --check`. No user dogfooding is required to start or continue independent work.

## Modernization priorities and deferred work

### User direction confirmed 2026-09-06

The ReShade bridge is temporary. Native display HDR, geometry detail, and lighting improvements must work independently of it. NR's apparent dynamic-range enhancement does not replace an HDR-capable presentation path. Ultrawide HUD placement is an immediate user priority.
