...
-
```
# 2026-09-06 / modernization foundation and ultrawide HUD

- Branch: `codex/modernization-foundation`, based on `76ff35b5`.
- Build identity: `neo/CMakeLists.txt` generates the exact target path per configuration. `Common.ps1::Find-NeuralDoomExecutable`, `Build-RBDOOM.ps1`, and `Run-RBDOOM.ps1` support an explicit build directory and remove stale/wrong-configuration fallback. Builds invalidate previous success metadata, fail on a missing target, and write hash/configuration/feature/commit metadata to the ignored build tree.
- Automation: `Test-NeuralBuildIdentity.ps1` exercises custom output selection, stale-output rejection, wrong-config isolation, false-success build handling, manifest invalidation, and PowerShell parsing. `Test-NeuralDoom-Smoke.ps1` runs bounded isolated gameplay/capture/reset/backend checks and records JSON evidence. It checks the existing Win32 command-line size limit rather than silently truncating its script invocation.
- Temporal inputs: `NeuralTemporal.h/.cpp::R_ValidateNeuralTemporalFrame` is shared by null and Streamline evaluation. It checks presence, viewport/storage extents, sample count, formats, input validity/conventions, camera ranges, finite matrices/vectors/jitter/exposure, exposure storage and input/output aliasing. Streamline records validation failure and leaves reset pending before falling back. This adds checks, not new resource formats or coordinate conventions: HDR/output RGBA16F, motion RG16F in previous-minus-current pixels, masks R8, device depth D24S8 or the existing D32S8 fallback, single-sample inputs. Native-resolution storage may contain smaller render extents.
- HUD: `MenuHandler_HUD.cpp::Initialize` opts its SWF into HUD layout. `SWF.h`, `SWF_Main.cpp`, `SWF_Render.cpp::Render/RenderSprite`, and `SWF_Events.cpp::HandleEvent` add a centered maximum HUD aspect and uniform scale with matching input-coordinate scale. Ordinary left/right/corner anchors use a horizontal inset in pixel-aspect-corrected screen units; root artwork remains centered and full-screen/absolute anchors retain their existing bounds. Defaults 0/1 preserve current layout. No world/viewmodel projection, shader or NR bridge change is involved.
- Captures: `RenderSystem_init.cpp::R_ReadPixelsRGB8` now honors `fs_savepath` on all platforms, keeping test output out of the game installation. PNG encoding and pixel format are unchanged.
- Both DX12 RelWithDebInfo configurations built using the existing VS2022/Windows SDK and pinned local Streamline configuration. Detailed commands/results and limitations are in `TEST_RESULTS.md` and usage is in `UNATTENDED_WORKFLOW.md`.
- Focused diff review checked resource bounds versus subrects, depth-format fallback, feature-off behavior, HUD instance isolation, full-screen overlays, command-line truncation, bounded process ownership, and exact build selection. No third-party dependency was added.
- Next: broader HUD notification/PDA/menu comparisons and contract negative-case coverage, then native HDR presentation capability diagnostics. Geometry relief and lighting experiments remain separate opt-in work.

## 2026-09-06 - Pre-playtest correctness review
...
- Added `R_BeginRayTracedReflections`, `PrepareRayTracedLighting` and `RayTracedLighting::{BeginReflections, InitializeReflections, RenderReflections, PrintReflectionStatus}` in `neo/renderer/RayTracingDiagnostic.cpp`. Reflection and GI share the existing BSP scene, material atlas, authored lights and pre-effect HDR radiance snapshot. Five reflection cvars participate in `R_RayTracingSettingsChanged`; debug modes 5/6 and `rayTracingReflectionStatus` are available. GI strength defaults to 1.125, down from 1.5.
- `RenderBackend.cpp::DrawViewInternal/DrawSingleInteraction`, `RenderProgs.cpp/.h`, `Framebuffer.h`, `NVRHI/Framebuffer_NVRHI.cpp` and `PipelineCache.cpp::GetOrCreatePipeline` route optional native material capture. Two IBL pixel shaders emit matched probe specular, BRDF response/roughness and world normals to three extra RGBA16F MRTs. Target 0 and the baseline shader outputs remain unchanged. Skinned capture variants retain joint bindings; ShaderMake macro ordering matches engine lookup ordering.
- New `neo/shaders/rt/reflections.cs.hlsl`, `reflection_filter.cs.hlsl`, `reflection_composite.cs.hlsl` and `reflection_parameters.hlsli` implement full-resolution rough ray queries, guided temporal/spatial filtering and native probe replacement. `ray_materials.hlsli` extracts the existing diffuse-bounce material/light/cache functions. `bounce_composite.cs.hlsl` restricts its diagnostic branch to modes 3/4 so reflection modes remain independent. CMake and shader configuration compile the added stages/permutations.
- Full resource formats, 192/112-byte constant layouts, coordinate conventions, rejection/reset behavior and limitations are recorded in [RAY_TRACED_REFLECTIONS.md](RAY_TRACED_REFLECTIONS.md). Ray/filter targets are full viewport resolution, with no half-resolution option. The material response is applied after filtering to preserve each current pixel's reflectivity. Misses preserve the native probe contribution; strength zero performs no scene write.
...
Validation: confirmed the game was closed; verified a unique active NR section, unchanged unrelated ReShade sections and game settings, and the final two lighting values. No renderer C++/shader/resource/coordinate changes, no rebuild needed, and no game was launched. Visual acceptance of the reduced blend and add-on defaults remains the next user comparison. The reflection control blends toward the native probe contribution; it is not a global exposure adjustment.
...
## 2026-09-07 — SWF hover lifetime fix
