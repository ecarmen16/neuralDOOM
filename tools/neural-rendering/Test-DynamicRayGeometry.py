...
CVar r_rayTracingDynamicGeometry,r_rayTracingSkinnedGeometry;
struct rayDynamicSurface_t {rayDynamicSurface_t* next=nullptr;idVec3* positions=nullptr;idVec2* texcoords=nullptr;uint32* indices=nullptr;int numVerts=0,numIndexes=0;bool castsShadow=true,skinned=false;};
...
uint32 staticVertexCount=17,dynamicSurfaceCount=0,dynamicTriangleCount=0,dynamicSkinnedCount=0,dynamicSkippedCount=0;
...
"""
checks=r"""
gotPoints=points;gotIndices=elements;
}
...
struct idMaterial {bool ReceivesLighting()const{return true;}int Coverage()const{return 0;}int Deform()const{return 0;}bool HasSubview()const{return false;}bool IsPortalSky()const{return false;}bool SurfaceCastsShadow()const{return true;}bool TestMaterialFlag(int)const{return false;}};
struct rayDynamicSurface_t {rayDynamicSurface_t* next;idVec3* positions;idVec2* texcoords;triIndex_t* indices;int numVerts,numIndexes;const idMaterial* material;const float* shaderRegisters;bool castsShadow,skinned;};
...
struct CVar {bool enabled=true;bool GetBool()const{return enabled;}};
CVar r_useGPUSkinning,r_rayTracingSkinnedGeometry;
